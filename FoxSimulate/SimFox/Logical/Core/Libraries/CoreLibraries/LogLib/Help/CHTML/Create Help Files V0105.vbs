...
'*   01.05       29.05.06    M.  Wietzke    changes to recognize '.svn'      *
'* DON'T FORGET TO UPDATE CONSTANT SCRIPT_VERSION_STRING !!!                 *
'*****************************************************************************
Option Explicit

Const SCRIPT_VERSION_STRING = "VBScript V1.05"	' WEPA20020727		
...
Private Function TreeSearch(pathName, baseFolder, fileEnable)
	...
	For i=1 To folderList.count
		...
		if (left(folder.name,1) <> "_"  and folder.name <> "images" and folder.name <> ".svn") then
